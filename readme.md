...
Начал кодить 4 года назад, начал писать на python, позже для изучения django попробывал в web(frontend), позже перешел на Java и начал изучать Spring. Нередко приходится использовать sql и postegsql. Недавно начал изучать парсинг сайтов, react, docker
